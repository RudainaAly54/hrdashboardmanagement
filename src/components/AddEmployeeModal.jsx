...
const AddEmployeeModal = ({isOpen, onClose, onSuccess}) =>{
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const dept = DEPARTMENTS.find((d) => d.name === form.deptName);
            const id = await generateNextId(dept.prefix);
            const photoUrl = pickAvatarUrl(form.email || form.fullName, form.gender)
           ...
           console.error("Error adding employee:", err);
            setError(err.message || "Something went wrong while adding the employee.");
        } finally {
            setSaving(false);
        }
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    <h2 className="text-lg font-bold font-[manrope]">Add Employee</h2>
