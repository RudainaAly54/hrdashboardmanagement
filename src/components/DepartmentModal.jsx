...
import { div, option } from "framer-motion/m";
...
const DeparmentModal = ({
    ...
    //employees fetching for selected department
    useEffect(() => {
        if(!isOpen) return 
        supabase.from("Employees")
        .select("id, fullName")
        .order("fullName")
        .then(({data}) => setEployees(data || []));
    }, [isOpen]);
...
//Fields 
    const fields = [
        ...
        {name: "headID", label: "Head of the Department", type: "select", span: 2, includeEmpty: true, emptyLabel: "Unassigned", 
            options: employees.map(emp => ({value: emp.id, lable: emp.fullName }))
        }
    ];
